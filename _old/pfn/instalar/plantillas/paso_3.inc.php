...
<h2><?php echo $conf->t('i:comprobacion'); ?></h2>

<br /><?php echo $conf->t('i:intro_comprobacion'); ?><br /><br />

<div class="capa_<?php echo $comprobar[0]; ?>">
	<strong>PHP > 4.0.6</strong><br />
	<?php echo $conf->t(($comprobar[0] == 'ok')?'i:instalado_ok':'i:instalado_erro').'<strong> '.phpversion(); ?></strong>
</div>
...
<div class="capa_<?php echo $comprobar[1]; ?>">
	...
	<?php } else { ?>
		<?php echo $conf->t(($comprobar[1] == 'ok')?'i:instalado_ok':'i:instalado_erro').'<strong> '.mysql_get_client_info(); ?></strong>
	<?php } ?>
</div>
...
<div class="capa_<?php echo $comprobar[2]; ?>">
	...
	<?php if ($erro_gd) { ?>
		<?php echo $conf->t('i:gd_erro'); ?>
	<?php } else { ?>
		<?php if ($comprobar[2] == 'ok') { ?>
			<input type="hidden" name="gd2" value="true" />
		<?php } ?>
		<?php echo $conf->t(($comprobar[2] == 'ok')?'i:instalado_ok':'i:instalado_aviso').'<strong> '.$gd_version[1]; ?></strong>
	<?php } ?>
</div>
...
<div class="capa_<?php echo $comprobar[3]; ?>">
	...
	<?php if ($comprobar[3] == 'ok') { ?>
		...
		<?php echo $conf->t('i:instalado_ok').'<strong> '.$zlib_version[1]; ?></strong>
	<?php } else { ?>
		<?php echo $conf->t('i:zlib_erro'); ?>
	<?php } ?>
</div>

<div class="capa_<?php echo $comprobar[4]; ?>">
	<strong>safe_mode = Off</strong><br />
	<?php echo $conf->t(($comprobar[4] == 'ok')?'i:safe_mode_ok':'i:safe_mode_erro'); ?>
</div>

<div class="capa_<?php echo $comprobar[5]; ?>">
	<strong>upload_max_filesize</strong><br />
	<?php echo $conf->t(($comprobar[5] == 'ok')?'i:upload_ok':'i:upload_erro').'<strong> '.$upload_max_filesize[0]; ?></strong>
</div>

<div class="capa_<?php echo $comprobar[6]; ?>">
	<strong>memory_limit</strong><br />
	<?php echo $conf->t(($comprobar[6] == 'ok')?'i:memory_ok':'i:memory_erro').'<strong> '.$memory_limit[0]; ?></strong>
</div>
...
<div style="text-align: right">
	<input type="submit" id="btn_enviar" value="<?php echo $conf->t('i:recargar'); ?>" />
	<br /><br /><div class="fondo_gris">
		<label for="omitir_paso"><?php echo $conf->t('i:omitir'); ?></label>
...
function activa_paso () {
	if (document.getElementById('omitir_paso').checked == true) {
		...
		document.getElementById('btn_enviar').value = '<?php echo PFN_quitaHtmlentities($conf->t('continuar_paso_'.(($form['tipo'] == 'instalar')?4:5))); ?>';
	} else {
		document.getElementById('paso').value = 3;
		document.getElementById('btn_enviar').value = '<?php echo PFN_quitaHtmlentities($conf->t('i:recargar')); ?>';
	}
}
...
//--></script>
<?php } else { ?>
<input type="submit" value="<?php echo $conf->t('continuar_paso_'.(($form['tipo'] == 'instalar')?4:5)); ?>" class="dereita" />
<?php } ?>
</fieldset>
</form>
...
<input type="hidden" name="ra_web" value="<?php echo $form['ra_web']; ?>" />
<input type="hidden" name="ra_dominio" value="<?php echo $form['ra_dominio']; ?>" />
<input type="submit" value="<?php echo $conf->t('voltar_paso_2'); ?>" class="esquerda" />
</fieldset>
</form>
