...
$usuario = $vars->post('activar_usuario');
$contrasinal = md5($vars->post('activar_contrasinal'));

include ($PFN_paths['plantillas'].'cab.inc.php');

if (empty($usuario) || empty($contrasinal)) {
	$txt_erro = $conf->t('activar_contrasinal_intro');
	...
	if ($usuarios->activar_contrasinal($usuario, $contrasinal)) {
		$txt_erro = $conf->t('activar_contrasinal_ok');
		include ($PFN_paths['plantillas'].'login.inc.php');
	} else {
		$txt_erro = $conf->t('activar_contrasinal_erro');
		include ($PFN_paths['plantillas'].'activar_contrasinal.inc.php');
	}
}
