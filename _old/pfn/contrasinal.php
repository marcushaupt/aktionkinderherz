...
if (($vars->post('recuperar_usuario') != '')
&& ($vars->post('recuperar_email') != '')) {
	...
	$ok = $usuarios->nova_contrasinal();

	if ($ok === 1) {
		$txt_erro = $conf->t('avisos_novo_contrasinal', 1);
		$usuarios->contrasinal_temporal();
	} else {
		$txt_erro = $conf->t('avisos_novo_contrasinal', $ok);
	}
} else {
	$txt_erro = $conf->t('txt_novo_contrasinal');
}
