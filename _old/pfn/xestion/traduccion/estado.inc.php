...
&& ereg('^[a-z]+$', $tr_idioma_base)
&& is_dir($PFN_paths['idiomas'].$niveles->path_correcto($tr_idioma_base))) {
	$path_base = $PFN_paths['idiomas'].$niveles->path_correcto($tr_idioma_base);

	$txt_informe = '<table class="tabla_informes" summary="">'
		."\n".'<tr><th>'.$conf->t('idioma').'</th>';
	...
	$txt_informe .= '<th>'.$conf->t('descargar').'</th>'
		.'</tr>'."\n".'<tr style="text-align: right;"><td>'
		.$conf->t('lista_idiomas',$tr_idioma_base).'</td>';
	...
	$txt_informe .= '<td><ul class="accions"><li class="descargar"><a href="descargar.php?'
		.PFN_cambia_url('idioma',$tr_idioma_base,false).'" title="'.$conf->t('descargar')
		.'"><span class="oculto">'.$conf->t('descargar').'</span></a></li></ul></td></tr>';
	...
	foreach ($idiomas_valen as $k => $v) {
		...
		$txt_informe .= '<td><ul class="accions"><li class="descargar"><a href="descargar.php?'
			.PFN_cambia_url('idioma',$k,false).'" title="'.$conf->t('descargar')
			.'"><span class="oculto">'.$conf->t('descargar').'</span></a></li></ul></td></tr>';
	}
