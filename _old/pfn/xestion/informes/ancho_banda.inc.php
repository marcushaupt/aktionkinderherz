...
$ab_mes = trim($vars->post('ab_mes'));
$ab_ano = trim($vars->post('ab_ano'));
$ab_ordenar = trim($vars->get('ab_ordenar'));
$ab_modo = trim($vars->get('ab_modo'));
...
$vars->get('ab_mes', $ab_mes);
$vars->get('ab_ano', $ab_ano);
...
for (; $usuarios->mais(); $usuarios->seguinte()) {
	$listado['id'][] = $usuarios->get('id');
	$listado['nome'][] = $usuarios->get('nome');

	if ($usuarios->get('descargas_maximo') > 0) {
		$limite = $usuarios->get('descargas_maximo');
	} else {
		$limite = -1;
	}

	if (is_file($PFN_paths['info'].'usuario'.$usuarios->get('id').'/descargas.'.$ab_ano.$ab_mes.'.php')) {
		$actual = include ($PFN_paths['info'].'usuario'.$usuarios->get('id').'/descargas.'.$ab_ano.$ab_mes.'.php');
...
$txt = '<table class="tabla_informes" summary="">'
	...
	.'<th style="text-align: left;">'
		...
		.'">'.$conf->t('Xcol_ancho_banda_limite').'</a></th>'
	.'<th style="text-align: left;">'
		...
		.'">'.$conf->t('Xcol_ancho_banda_actual').'</a></th>'
	.'<th style="text-align: left;">'
		...
		.'">'.$conf->t('Xcol_porcent_libre').'</a></th></tr>';
...
foreach ((array)$listado[$ab_ordenar] as $k => $v) {
	...
	if ($listado['limite'][$k]) {
		...
		$txt .= '<td>'.(($listado['limite'][$k] === -1)?$conf->t('sen_limite'):PFN_peso($listado['limite'][$k])).'</td>'
		...
		$txt .= '<td colspan="3">'.$conf->t('sen_datos').'</td></tr>';
	}
}
