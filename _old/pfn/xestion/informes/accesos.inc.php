...
$ae_buscar = trim($vars->post('ae_buscar'));
$ae_lineas = intval($vars->post('ae_lineas'));
$ae_lineas = ($ae_lineas < 1 || $ae_lineas > 500)?50:$ae_lineas;
$ae_amosar = is_array($vars->post('ae_amosar'))?$vars->post('ae_amosar'):array();
...
$txt = '<table class="tabla_informes" summary="">'
	.'<tr><th>'.$conf->t('Xcol_data').'</th>'
	.'<th>'.$conf->t('Xcol_login').'</th>'
	.'<th>'.$conf->t('Xcol_ip').'</th>'
	.'<th>'.$conf->t('Xcol_estado').'</th>'
	.'<th>'.$conf->t('Xcol_donde').'</th>'
	.'<th>'.$conf->t('Xcol_sesion').'</th></tr>';

for ($usuarios->informe($w); $usuarios->mais(); $usuarios->seguinte()) {
	if ($usuarios->get('estado') == '1') {
		$class = 'tr_ok';
	} else {
		$class = 'tr_ko';
	}

	$txt .= '<tr class="'.$class.'"><td>'.date("Y/m/d H:i:s", $usuarios->get('data')).'</td>'
		.'<td>'.$usuarios->get('login').'</td>'
		.'<td>'.$usuarios->get('ip').'</td>'
		.'<td>'.$usuarios->get('estado').'</td>'
		.'<td>'.$usuarios->get('donde').'</td>'
		.'<td>'.$usuarios->get('session').'</td></tr>';
}
