...
$ai_raiz = intval($vars->post('ai_raiz'));
$ai_buscar = trim($vars->post('ai_buscar'));
$ai_lineas = intval($vars->post('ai_lineas'));
$ai_lineas = ($ai_lineas < 1 || $ai_lineas > 500)?50:$ai_lineas;
$ai_amosar = is_array($vars->post('ai_amosar'))?$vars->post('ai_amosar'):array();
...
$lineas = $ai_lineas;
$info_raiz = $niveles->path_correcto($PFN_paths['info'].'raiz'.$ai_raiz);

if ($conf->g('logs','accions') && is_file($info_raiz.'/'.$conf->g('logs','accions'))) {
	include_once ($info_raiz.'/'.$conf->g('logs','accions'));
	...
	if (count($log)) {
		...
		$txt = '<table class="tabla_informes" summary="">'
			...
			.'<th>'.$conf->t('Xcol_usuario').'</th>'
			.'<th>'.$conf->t('Xcol_accion').'</th>'
			.'<th>'.$conf->t('Xcol_arquivo').'</th></tr>';
		...
		foreach ($log as $k => $v) {
			...
			$txt .= '<tr'.((($b % 2) == 0)?' class="tr_par"':'').'><td>'.$partes[1][0].'</td>'
				.'<td>'.$partes[2][0].'</td>'
				.'<td>'.$conf->t('Xamosar_'.$partes[3][0]).'</td>'
				.'<td>'.$partes[4][0].'</td></tr>';
