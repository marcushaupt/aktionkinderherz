...
$ud_ordenar = trim($vars->get('ud_ordenar'));
$ud_modo = trim($vars->get('ud_modo'));
...
for (; $usuarios->mais(); $usuarios->seguinte()) {
	...
	if ($usuarios->get('peso_maximo') > 0) {
		$actual = $usuarios->get('peso_actual');
		$limite = $usuarios->get('peso_maximo');
...
foreach ((array)$listado[$ud_ordenar] as $k => $v) {
	...
	if ($listado['limite'][$k]) {
		...
		$txt .= '<td colspan="3">'.$conf->t('sen_limite').'</td></tr>';
	}
}
