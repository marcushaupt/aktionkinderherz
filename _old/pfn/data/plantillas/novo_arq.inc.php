...
?>
<div id="ver_info">
	<div class="bloque_info"><h1><?php echo $conf->t('accion').' &raquo; '.$conf->t('novo_arq'); ?></h1></div>
	<div class="bloque_info" style="text-align: center;">
		...
		&nbsp;&nbsp;
		<input type="submit" name="cambiar_tamano" value="<?php echo $conf->t('cambiar'); ?>" />
		</fieldset>
		</form>
		...
		<br />
		<textarea id="texto" name="texto" style="width: <?php echo $editar_ancho; ?>px; height: <?php echo $editar_alto; ?>px"><?php echo htmlentities($vars->post('texto2'), ENT_NOQUOTES, $conf->g('charset')); ?></textarea>
		<br /><br />
		<table class="tabla_info" summary="">
			<tr>
				<th><label for="nome_arq"><?php echo $conf->t('nome_arq'); ?></label>:</th>
				<td><input type="text" name="cal" id="nome_arq" value="" class="text" /></td>
			</tr>
			<?php foreach ($inc->crea_formulario('arq') as $v) { ?>
			<tr>
			...
			<?php } ?>
			<tr>
				<th><label for="sobreescribir"><?php echo $conf->t('sobreescribir'); ?></label>:</th>
