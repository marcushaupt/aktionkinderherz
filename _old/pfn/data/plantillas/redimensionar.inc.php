...
?>
	<div style="text-align: center;">
		...
		<fieldset>
		<input type="submit" name="cancelar" value=" <?php echo $conf->t('cancelar'); ?> " class="boton" />

		<br /><br />

		<div class="bloque_info" style="text-align: left;"><h1><?php echo $conf->t('accion').' &raquo; '.$conf->t('recortar'); ?></h1></div>
		<div class="aviso_info"><?php echo $conf->t('como_recortar'); ?></div>

		<script type="text/javascript"><!--

		var spacer = '<?php echo $conf->g('estilo'); ?>imx/semitrans.png';
		...
		<br /><br />
		<input type="submit" value="<?php echo $conf->t('aceptar'); ?>" class="boton" id="submit" onclick="return my_Submit('recortar');" />
		<br /><br />

		<div class="aviso_info"><?php echo $conf->t('escoller_reducida'); ?></div>
		<br />
		<a href="#"><img src="crea_imaxe.php?cal=<?php echo $ucal; ?>&amp;dir=<?php echo $dir; ?>&amp;ancho=<?php echo $conf->g('imaxes','ancho'); ?>&amp;alto=<?php echo $conf->g('imaxes','alto'); ?>&amp;<?php echo session_name().'='.session_id(); ?>" alt="<?php echo $vars->get('cal'); ?>" onclick="my_Submit('reducir');" /></a>

		<br /><br />

		<input type="submit" name="cancelar2" value=" <?php echo $conf->t('cancelar'); ?> " class="boton" />
		</fieldset>
		</form>
		<?php if ($hai_pequena) { ?>
		<br />

		<div class="aviso_info"><?php echo $conf->t('eliminar_reducida'); ?></div>
		<br />
		...
		<form action="accion.php?<?php echo PFN_cambia_url(array('cal','accion'),array($cal,'redimensionar'),false); ?>" method="post" onsubmit="return submitonce();">
			...
			<input type="submit" value="<?php echo $conf->t('eliminar'); ?>" class="boton" />
		</form>
		<?php } ?>
		<br /><br />
	</div>
...
function my_Submit (modo) {
	destino = 'accion.php?modo='+modo+'&accion=redimensionar&executa=true' +
		'&cal=<?php echo $ucal; ?>' +
		'&dir=<?php echo urlencode($dir); ?>&ver_imaxes=<?php echo $vars->get('ver_imaxes'); ?>' +
		'&<?php echo session_name().'='.session_id(); ?>' +
		'<?php
			for ($i = 0, $cnt = 0; $i < $conf->g('inc','limite'); $i++) {
				if ($vars->get("mais_$i") != '') {
					echo "&mais_$cnt=".urlencode($vars->get("mais_$i"));
					$cnt++;
				}
			}
		?>';
