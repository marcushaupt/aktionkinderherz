...
foreach ($partes as $k => $v) {
	if (!empty($v)) {
		...
		if ($v == '.') {
			echo ' <a href="navega.php?'.PFN_cambia_url('dir','.',false).'">'.$conf->t('comezo').'</a> /';
		} else {
...
?>
	...
	function envia_busca (obx_form) {
		...
		if (obx_palabra == "" || obx_palabra == "<?php echo $conf->t('busca'); ?>") {
			return false;
		}
	...
	<div id="buscador">
		...
		<input type="text" name="palabra_buscar" id="palabra_buscar" value="<?php echo $conf->t('busca'); ?>" onfocus="this.value='';" />
		<input type="image" name="submit" src="<?php echo $conf->g('estilo'); ?>imx/buscar.png" style="border: 0;" />
		</fieldset>
		</form>
	</div>
	<?php } ?>
