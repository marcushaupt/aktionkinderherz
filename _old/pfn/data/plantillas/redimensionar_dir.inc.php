...
?>
		<div class="bloque_info"><h1><?php echo $conf->t('accion').' &raquo; '.$conf->t('redimensionar'); ?></h1></div>
		<div class="aviso">
			<?php if ($vale) { ?>
			...
			<p><?php echo $conf->t('redimensionar_dir_txt'); ?></p>
			...
			&nbsp;&nbsp;<label for="sobreescribir"><?php echo $conf->t('sobreescribir_reducidas'); ?></label>

			<br /><input type="checkbox" name="previsualizar" id="previsualizar" value="si" />
			&nbsp;&nbsp;<label for="previsualizar"><?php echo $conf->t('amosar_previsualizacions'); ?></label>

			<br /><br />

			<input type="reset" value=" <?php echo $conf->t('cancelar'); ?> " class="boton" id="btn_reset" onclick="enlace('navega.php?<?php echo PFN_get_url(false); ?>');" />
			<input type="submit" value=" <?php echo $conf->t('aceptar'); ?> " class="boton" id="btn_submit" style="margin-left: 40px;" />
			</fieldset>
			</form>
			<?php } else { ?>
			<?php echo $conf->t('estado.redimensionar_dir',0); ?>
			<?php } ?>
		</div>
