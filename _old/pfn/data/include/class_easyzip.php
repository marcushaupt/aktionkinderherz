...
class PFN_EasyZIP {
	...
	/**
	* function EasyZIP (object &$conf)
	*
	* Inicializa la clase y carga el objecto $conf de configuraciónes
	*/
	function PFN_EasyZIP (&$conf) {
		$this->conf = &$conf;
	}
	...
	Function listar_contido () {
		...
		while (true) {
			...
			if ($signature == $this->zipSignature) {
				...
				$extraFieldLength[1]?fread($fh, $extraFieldLength[1]):'';
