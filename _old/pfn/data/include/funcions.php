...
function PFN_textoInterno2Form ($d) {
	...
	$d = eregi_replace('(<br>|<br />)', "\n", $d);
	return htmlentities($d, ENT_NOQUOTES, $conf->g('charset'));
}
...
function PFN_textoArquivo2pantalla ($d, $php=false) {
	...
	if ($php) {
		...
		$d = htmlentities($d, ENT_NOQUOTES, $conf->g('charset'));
		$d = split("(\r\n|\n|\r)", $d);
		$texto = '';
		$total = strlen(count($d));
...
function PFN_listado_select ($total, $actual) {
	...
	if ($total == 0) {
		...
		if ($total > $paxinar) {
			...
			$cad .= "\n".'<option value="-1" '.$select.'>'.$conf->t('TODO').'</option>';
		}
	}
