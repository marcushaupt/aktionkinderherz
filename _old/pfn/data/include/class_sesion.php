...
class PFN_Sesion {
	...
	*/
	function PFN_Sesion (&$conf) {
		...
		$this->vars = &$vars;
		$this->tabla = $conf->g('db','prefixo').'sesions';
	...
	*/
	...
	function ler () {
		...
		$clases->FILE = __FILE__;
		$clases->LINE = __LINE__+1;
		...
		$clases->lock($this->tabla, 'READ');
		$ok = $clases->put_query($query);
		$clases->unlock();


		if ($ok > 0) {
			$contido = $clases->get('contido');
	...
	*/
	function escribir ($id, $sess_data) {
		...
		$clases->lock($this->tabla, 'WRITE');
		$clases->actualizar($query);
		$clases->unlock();
