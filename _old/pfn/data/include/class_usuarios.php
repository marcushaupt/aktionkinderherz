...
class PFN_Usuarios extends PFN_Clases {
	...
	*/
	function PFN_Usuarios (&$conf) {
		...
		$this->tablas['sesions'] = $conf->g('db','prefixo').'sesions';
		$this->tablas['bloqueo'] = $conf->g('db','prefixo').'bloqueo_ip';
		...
		switch ($modo) {
			...
			case 'session':
				...
				$this->query = 'SELECT '.$this->tablas['raices'].'.*'
					...
					.', '.$this->tablas['bloqueo'].', '.$this->tablas['sesions']
					...
					.' AND '.$this->tablas['sesions'].'.id = "'.addslashes(session_id()).'"'
