...
class PFN_Niveles {
	...
	/**
	* function PFN_Niveles (object &$conf)
	*
	* recibe el objecto $conf con los parametros de configuración
	*/
	function PFN_Niveles (&$conf) {
		$this->conf = &$conf;
	}

	/**
	* function imaxes (object &$conf)
	*
	* recibe el objecto $conf con los parametros de configuración
	*/
	function imaxes (&$imaxes) {
		$this->imaxes = &$imaxes;
	}
	...
	*/
	...
	/**
	* function get_contido (string $nivel, string $orde, string $pos, boolean $arbore)
	* $nivel: directorio a leer
	* $orde: campo por el cual ordenar el contenido
	* $pos: orden Ascendente o Descendente
	* $arbore: si esta activa solo carga el nombre y evita cargar toda la
	...
	* return array
	*/
	function get_contido ($nivel, $orde='nome', $pos='ASC', $arbore=false) {
		$lista = &$this->carga_contido($nivel,true,$arbore);
	...
	/**
	* function carga_contido (string $nivel, boolean $completo, boolean $arbore)
	* $nivel: directorio a leer
	...
	* return array
	*/
	function carga_contido ($nivel, $completo=true, $arbore=false) {
		$this->nivel = $nivel;
	...
	*/
	function nome_correcto ($nome) {
		return ereg_replace('\.{2,}','', PFN_check_nome($nome));
	}
