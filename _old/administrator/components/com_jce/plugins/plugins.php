...
require_once( $mosConfig_absolute_path.'/administrator/components/com_jce/plugins/plugins.html.php' );
...
function editPlugins( $option, $uid, $client ) {
	...
	if ($uid) {
		...
		// xml file for module
		$xmlfile = $mosConfig_absolute_path . '/mambots/editors/jce/jscripts/tiny_mce/plugins/' .$row->plugin . '/' . $row->plugin .'.xml';
		...
		$folders = mosReadDirectory( $mosConfig_absolute_path . '/mambots/editors/jce/jscripts/tiny_mce/plugins/' );
		$folders2 = array();
		foreach ($folders as $folder) {
			if (is_dir( $mosConfig_absolute_path . '/mambots/editors/jce/jscripts/tiny_mce/plugins/' . $folder ) && ( $folder <> 'CVS' ) ) {
